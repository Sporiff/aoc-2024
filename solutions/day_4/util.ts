...
export const rearrangeDiagonalLines = (input: string): string => {
    ...
    const getDiagonals = (startRow: number, startCol: number, direction: "right" | "left") => {
        const diagonals: string[] = [];
        let row = startRow;
        let col = startCol;

        while (row < numRows && col >= 0 && col < numCols && col < lines[row]?.length) {
            let diagonal = "";
...
export function findXmas(input: string): number {
    ...
    function findX(lineNumber: number, characterPosition: number): boolean {
        ...
        if (lines[lineNumber].length <= characterPosition + 2 ||
